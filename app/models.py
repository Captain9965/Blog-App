...
class User(UserMixin, db.Model):   # inherits from db.Model, a base class for all models from Flask-SQLAlchemy.
    ...
    def followed_posts(self):#create temporary table by join method according to condition, then filter, then order
        followed = Post.query.join(
            followers, (followers.c.followed_id == Post.user_id)).filter(
                followers.c.follower_id == self.id)
        own = Post.query.filter_by(user_id=self.id)
        return followed.union(own).order_by(Post.timestamp.desc())  #union operator
